...
class Phy:
    ...
    def __init__(self, mac):
        ...
        # Choose default TX power level (not used yet)
        if self.profile.tx_power_levels:
            # Choose which TX power level to use
            self.current_tx_power_label = "low" if "medium" in self.profile.tx_power_levels else list(self.profile.tx_power_levels.keys())[0]
        ...
        # Debug 
        # print(f"[PHY INIT] Drone {getattr(self.my_drone, 'identifier','?')} using {self.profile.name} | TX level={self.current_tx_power_label} ({self.current_tx_power_dbm} dBm)")
    ...
    def rx_energy(self, packet):
        """
        Placeholder for RX energy usage.
        Not connected yet — future integration
...
if __name__ == "__main__":
    print("------------------------------------------------------")
    print(" STANDALONE PHY POWER-MODE PLACEHOLDER TEST")
    print("------------------------------------------------------")
