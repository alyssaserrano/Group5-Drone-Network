...
class CsmaCaV2:
    ...
    # Beaconing (v2)
    # -----------------
    def _beacon_loop(self, initial_delay_us):
        # initial jitter
        yield self.env.timeout(initial_delay_us)
        while True:
            # Gentle contention for beacon: a small random delay before sending
            gentle_slots = max(1, int((getattr(config, "CW_MIN", 8)) // 2))
            bo = self.rng_mac.randint(0, gentle_slots)
            # wait until idle, then small DIFS, then bo slots
            yield self.env.process(self.wait_idle_channel(self.my_drone, self.simulator.drones))
            yield self.env.timeout(config.DIFS_DURATION)
            for _ in range(bo):
                # if channel gets busy, restart next round
                if not check_channel_availability(self.channel_states, self.my_drone, self.simulator.drones):
                    break
                yield self.env.timeout(config.SLOT_DURATION)

            # Transmit a beacon: try to use an existing helper if present; otherwise, send a tiny broadcast frame
            beacon_pkd = None
            if hasattr(self.my_drone, "generate_beacon_packet"):
                try:
                    beacon_pkd = self.my_drone.generate_beacon_packet()
                except Exception:
                    beacon_pkd = None

            beacon_bits = self.beacon_length_bits
            tx_time_us = beacon_bits / config.BIT_RATE * 1e6

            with self.channel_states[self.my_drone.identifier].request() as req:
                yield req
                if beacon_pkd is not None:
                    self.phy.broadcast(beacon_pkd)
                else:
                    # Fallback: occupy channel for the beacon airtime (no payload expectations)
                    # This preserves "beaconing" behavior without requiring a specific Packet API.
                    pass
                logger.info('At time: %s (us) ---- UAV: %s (v2) TX BEACON',
                            self.env.now, self.my_drone.identifier)
                yield self.env.timeout(tx_time_us)

            # sleep until next interval
            yield self.env.timeout(self.beacon_interval_us)
